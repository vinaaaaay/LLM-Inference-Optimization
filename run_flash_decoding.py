...
Model: facebook/opt-350m (331M params, 16 heads, head_dim=64)
...
# Model attention config (OPT-350m)
NUM_HEADS = 16
HEAD_DIM = 64
...
# ---------------------------------------------------------------------------
def make_decode_qkv(batch_size, num_heads, kv_len, head_dim, dtype=torch.float16):
    """
    Create tensors simulating a single-token decode step:
      Q: (batch, heads, 1, head_dim)      — the new token query
      K: (batch, heads, kv_len, head_dim)  — cached keys
      V: (batch, heads, kv_len, head_dim)  — cached values
    """
...
def run_exp4_kernel_decode(args, tokenizer, results_dir):
    ...
    Benchmarks raw F.scaled_dot_product_attention with:
      Q: (1, 16, 1, 64)    — single token query
      K: (1, 16, kv_len, 64) — varying KV cache sizes
      V: (1, 16, kv_len, 64)
    ...
    for kv_len in KERNEL_KV_LENGTHS:
        q, k, v = make_decode_qkv(batch_size, NUM_HEADS, kv_len, HEAD_DIM)
        for bname, benum in BACKENDS.items():
    ...
    if all_results:
        ...
        write_csv(all_results, results_dir / "exp4_kernel_decode.csv",
                  cols + ["num_heads", "head_dim", "num_runs"])
        write_raw_csv(
            all_results,
...
def run_exp5_e2e_memory(args, tokenizer, results_dir):
    ...
    for seq_len in mem_seq_lengths:
        ...
        q, k, v = make_decode_qkv(batch_size, NUM_HEADS, seq_len, HEAD_DIM)
        for bname, benum in BACKENDS.items():
