...
MAX_SEQ_LEN = 512
LDG_LM_NUM_BLOCKS = 1184  # tuning needed based on model size and GPU
...
torch::Tensor build_layer_weights(
    std::vector<torch::Tensor> weights_flat,
    int num_layers
) {
    ...
    auto d_weights = torch::empty(
        {num_layers * (int)sizeof(LDGLayerWeights)},
        torch::dtype(torch::kUInt8).device(torch::kCUDA));
    cudaMemcpy(
        d_weights.data_ptr(), lw.data(),
...
class Decoder:
    ...
    def generate(self, prompt, max_new_tokens=100):
        ...
        next_token = self.step(input_ids[-1])
        generated = []
        ...
        for _ in range(max_new_tokens):
            ...
            if next_token == self.tokenizer.eos_token_id:
                break
            next_token = self.step(next_token)
